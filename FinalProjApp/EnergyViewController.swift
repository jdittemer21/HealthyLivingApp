...
class EnergyViewController: UITableViewController{
    ...
    override func viewDidLoad()
    {
        ...
        func shareButtonPressed(_sender: UIButton) {
            
            let alert = UIAlertController(title: "Share", message: "Share your results! Only works if you have Twitter.", preferredStyle: .actionSheet)
            ...
            let twitterAction = UIAlertAction(title: "Share on Twitter", style: .default) { (action) in
                ...
                if SLComposeViewController.isAvailable(forServiceType: SLServiceTypeTwitter) {
                    ...
                    post.setInitialText("I used the HealthyLiving App to help me with my daily dietary needs!")
            ...
            let addBarButton: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.add, target: self, action: #selector(EnergyViewController.refreshStatistics));
            ...
            let completeSecondTime: ((Double, Error?) -> Void) = {
                ...
                let completion: ((HKQuantity?, Error?) -> Void) = {
                    ...
                    guard let basalEnergyBurn = basalEnergyBurn else {
                        print("An error occurred trying to compute the basal energy burn. Error: \(error)")
    ...
    // Calculates the user's total resting energy burn based off of their height, weight, age,
    // and gender
    ...
    func calculateBasalBurnTodayFromWeight(_ weight: HKQuantity?, height: HKQuantity?, dateOfBirth: Date, biologicalSex: HKBiologicalSexObject) -> HKQuantity?
    {
